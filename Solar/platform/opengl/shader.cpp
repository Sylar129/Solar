...
std::optional<uint32_t> Compile(const ShaderSources& shader_sources) {
  SOLAR_PROFILE_FUNCTION();
...
OpenGLShader::OpenGLShader(const std::string& filepath) {
  ...
  auto id = Compile(shader_sources);
  if (id.has_value()) {
    renderer_id_ = id.value();
  } else {
    SOLAR_CORE_ASSERT(false, "Compile Failed!");
  }
