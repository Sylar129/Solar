...
namespace Solar {
    ...
    void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height) {
        if (width == 0 || height == 0 || width > s_MaxFramebufferSize || height > s_MaxFramebufferSize) {
            SOLAR_CORE_WARN("Attempted to resize framebuffer to {0}, {1}", width, height);
            return;
        }
    ...
    uint32_t OpenGLFramebuffer::GetColorAttachmentRendererID(uint32_t index) const {
        SOLAR_CORE_ASSERT(index < m_ColorAttachments.size(), "Index shoule be less than size!");
    ...
    void OpenGLFramebuffer::Invalidate() {
        ...
        if (m_ColorAttachments.size() > 1) {
            SOLAR_CORE_ASSERT(m_ColorAttachments.size() < 4, "The size of ColorAttachments should less than 4!");
        ...
        SOLAR_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Framebuffer is incomplete!");
